...
app.delete("/delete/:id", (req, res) => {
  ...
  const searchIndex = jokes.findIndex((index) => index.id === id);
  if (searchIndex > -1) {
    const firstHalf = jokes.slice(0, searchIndex);
    const secondHalf = jokes.slice(searchIndex + 1);
    jokes = firstHalf.concat(secondHalf);
    res.sendStatus(200);
  } else {
    res.status(404).json({ error: `No can do` });
  }
});
